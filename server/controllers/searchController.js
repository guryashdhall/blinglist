...
exports.searchProducts = async (req, res) => {
    try {
        ...
        if (data.length > 0) {
            ...
            return res.status(200).json({ data: result, success: true, message: "Products fetched with keyword: " + keyword + " successfully" });
        } else if (data.length == 0) {
            return res.status(404).json({ message: "Product with keyword '" + keyword + "' not found", data: [], success: true });
        } else {
