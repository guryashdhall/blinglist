...
const Favourites = () => {
  ...
  const handleSearchSubmit = async (e) => {
    ...
    if (keyword.length > 0) {
      console.log("searching:" + keyword)
      const res = await axios.get(`${BACKEND_URL}search/favourites?id=${user._id}&keyword=${keyword}`)
      ...
      <Container maxWidth="lg">
        ...
        <Box sx={{ flexGrow: 1, mx: "auto", mt: 4 }}>
          <Grid
            ...
            columns={{ xs: 4, sm: 8, md: 12 }}
          >
            {favourites == undefined ? <h3>You do not have any products added to Wishlist</h3> :
              filterApplied ?
